...
@Component({
    selector: 'queue',
    templateUrl: '/owcs/app/components/queue/sq-districtOffice.html'
})
export class SQDistrictOfficeComponent implements OnInit {
...
this.queueService.fetchSharedQueue('distOfficeQueue').subscribe(
             (response: Response) => {
               this.data = response.json().getDistOfficeQueueOutput; 
